...
export default function FeaturesMinimalBlock(props: FeaturesMinimalBlockProps) {
  ...
  return (
    <section
      ...
      className={cn('px-4 xl:px-10 bg-lab-surface', {
        'border-t border-lab-border': enableBorderTop,
        'border-b border-lab-border': enableBorderBottom,
        'rounded-t-4xl': cornerRadiusTop === 'rounded',
        'rounded-b-4xl': cornerRadiusBottom === 'rounded'
      })}
    >
      <Container className='py-16 md:py-28 border-x border-lab-border border-dashed space-y-10 md:space-y-14'>
        <div className='grid grid-cols-12 gap-y-12 md:gap-y-20 xl:gap-x-20'>
          <div className='col-span-12 xl:col-span-5 max-w-[400px] md:max-w-full space-y-10 md:space-y-10'>
            <div className='lg:flex justify-between xl:flex-col'>
              <Heading tag="h2" size="xl" className='max-w-[420px] relative pr-2.5 py-3 text-balance leading-normal border-y border-lab-border border-dashed bg-lab-surface pattern-bg--2 text-slate-100'>
              ...
              {content && (
                <PortableTextEditor 
                  data={content}
                  classNames='max-w-[400px] mt-8 text-balance text-slate-400'
                />
              )}
            </div>
          ...
          <div className='col-span-12 xl:col-span-7'>
            <div className='grid md:grid-cols-2 gap-y-3 md:gap-x-10'>
              {features?.map((feature: string) => (
                <div key={feature} className='pb-3.5 flex items-center gap-3.5 border-b border-lab-border border-dashed'>
                  <Check size={18} className='text-lab-cyan shrink-0' />
